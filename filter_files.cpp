                          ...
                          const std::string& hash_algo, size_type block_size)
{
    ...
    std::unordered_set<decltype(fs::hash_value(path_t{}))> paths_to_ommit;
    for(const auto& folder:to_ommit)
        paths_to_ommit.insert(fs::hash_value(fs::absolute(folder)));
    ...
    for(const auto& folder:to_discover)
    {
        try
        {
            ...
            const auto phash{fs::hash_value(path)};
            auto el{paths_to_ommit.find(phash)};
